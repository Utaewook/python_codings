...
class Mtrx:  # 행렬의 합/차/곱 연산을 위한 행렬 클래스
    def __init__(self, name, n_row, n_col, L_data):  # 행렬 객체의 생성자
        ...
        self.mat_data = [[L_data[i + n_col * j] for i in range(self.n_col)] for j in
                         range(self.n_row)]  # 편리한 행렬 계산을 위한 2차원 배열로 변경
    ...
    def __str__(self):  # 행렬의 출력을 위한 문자열 반환
        ...
        for i in range(self.n_row):  # 행렬 열의 갯수 만큼 반복하는 반복문
            for j in range(self.n_col):  # 행렬 행의 갯수 만큼 반복하는 반복문
                s += f'\t{self.mat_data[i][j]:.2f}'  # 인덱스에 맞는 행렬의 값을 형식에 맞추어 초기 문자열에 더해준다
    ...
    def transpose(self):  # 전치 행렬을 반환하는 함수
        data = list(zip(*self.mat_data))  # 전치된 2차원 배열을 생성
        data_unpack = [data[row][col] for row in range(self.n_col) for col in range(self.n_row)]  # 생성된 전치 행렬을 1차원 배열로 만든다.
        return Mtrx(None, self.n_col, self.n_row, data_unpack)  # 생성된 1차원 배열을 통해 Mtrx 객체를 만들어 반환
