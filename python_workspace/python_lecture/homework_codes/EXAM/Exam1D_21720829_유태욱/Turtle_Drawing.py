...
# 작성자: 유태욱
# 작성일자: 2022-10-23
# 주요기능: 지정된 정다각형의 이름/꼭지점개수/turtle그리기를 반환 및 수행하는 함수
# 최종수정일자: 2022-10-23
# 수정내용: 최초작성
...
def drawPolygon(t, center_pos, num_vert, radius, color): # 다각형을 그리기 위한 매개변수들
    t.pencolor(color) # 다각형의 펜 색상 입력받은 색상으로 설정
    angle = 180*(1.0 - 2.0 / num_vert) # 다각형의 내각
    line_length = 2 * radius * math.cos(math.radians(angle/2))  # 다각형의 한 변의 길이를 구함
    t.penup() # 선 없이 이동하기 위한 penup
    t.goto(center_pos[0],center_pos[1]) # 중심점으로 이동한다.
    t.heading() # 원래 각도로 바라보기 위한 초기화
    t.dot(10, "blue"); t.write(t.pos()) # t 현재위치 좌표 찍기
    ...
    # t.write(f"{color} {getPolygonName(num_vert)}")
    for i in range(num_vert): # 입력받은 내각의 갯수 만큼 반복문
